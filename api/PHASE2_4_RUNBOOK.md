...
1. Set `ADMIN_API_KEY`, `ADMIN_USERNAME`, and `ADMIN_PASSWORD` in `.env` and **restart the API**
2. Open `http://localhost:4000/admin` (must be from the same host as the API)
3. Enter the admin username and password exactly as in `.env` and click Continue
4. View routers, vouchers, and sessions

Render/local env required for admin login:

```bash
ADMIN_USERNAME=your_admin_username
ADMIN_PASSWORD=your_admin_password
ADMIN_API_KEY=your_existing_admin_api_key
```
