 ...
 * Admin dashboard login can exchange ADMIN_USERNAME/ADMIN_PASSWORD for that key.
 */
...
r.post('/login', (req, res) => {
  ...
  const configuredUsername = (process.env.ADMIN_USERNAME || process.env.ADMIN_USER || '').trim();
  const configuredPassword = (process.env.ADMIN_PASSWORD || '').trim();
  const { username = '', password = '' } = req.body || {};

  if (!adminKey || !configuredUsername || !configuredPassword) {
    return res.status(503).json({ ok: false, error: 'Admin login not configured' });
  }

  if (String(username).trim() !== configuredUsername || String(password) !== configuredPassword) {
