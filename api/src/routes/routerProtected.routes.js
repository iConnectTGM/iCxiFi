...
function voucherScopeQuery({ code, tenantId, routerId }) {
  if (tenantId) {
    return { code, $or: [{ tenantId }, { routerId }] };
  }
  return { routerId, code };
}

function voucherGrantPayload(voucher) {
  const downloadKbps = voucher.downloadKbps || DEFAULT_DOWNLOAD_KBPS;
  const uploadKbps = voucher.uploadKbps || DEFAULT_UPLOAD_KBPS;
  const downloadQuotaKB = Number.isFinite(voucher.downloadQuotaKB) ? voucher.downloadQuotaKB : 0;
  const uploadQuotaKB = Number.isFinite(voucher.uploadQuotaKB) ? voucher.uploadQuotaKB : 0;
...
function voucherErrorResponse(res, existing, now) {
  if (!existing) {
...
// Phase 3.2
r.post('/vouchers/redeem/prepare', routerAuth, async (req, res) => {
  try {
    const profile = sanitizeProfile(req.router.profile);
    const routerId = req.router.routerId;
    if (isRateLimited(routerId, 'redeem_prepare', profile.limits.maxRedeemPerMinute)) {
      return res.status(429).json({ ok: false, error: 'Rate limit exceeded' });
    }
    ...
    const pendingExpiresAt = new Date(now.getTime() + 2 * 60 * 1000);
    const scopeQuery = voucherScopeQuery({ code, tenantId, routerId });
    ...
    if (!reserved) {
      const existing = await Voucher.findOne(scopeQuery).lean();
      return voucherErrorResponse(res, existing, now);
    }

    return res.json({
      ok: true,
      redeemToken,
      grant: voucherGrantPayload(reserved),
      voucher: {
        code: reserved.code,
...
r.post('/vouchers/redeem/commit', routerAuth, async (req, res) => {
  try {
    const routerId = req.router.routerId;
    const code = req.body && req.body.code ? String(req.body.code).trim().toUpperCase() : '';
    ...
    const grant = voucherGrantPayload(redeemed);
...
r.post('/vouchers/redeem/cancel', routerAuth, async (req, res) => {
  try {
    const routerId = req.router.routerId;
    const code = req.body && req.body.code ? String(req.body.code).trim().toUpperCase() : '';
    const redeemToken = req.body && req.body.redeemToken ? String(req.body.redeemToken).trim() : '';
    ...
    const result = await Voucher.updateOne(
      {
        ...voucherScopeQuery({ code, tenantId, routerId }),
        status: 'pending',
        pendingToken: redeemToken,
        pendingRouterId: routerId
      },
